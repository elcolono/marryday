...
export default function AccountUpdateProduct(pageProps) {
    ...
    const activeStep = router.query['step']
        ...
        : 0
    const isLastStep = activeStep === productBasicFields.length - 1;
    const currentValidationSchema = validationSchema[activeStep];
    const formInitialValues = product

    function _renderStepContent(step, values) {
        return <FormFieldsGenerator data={productBasicFields[activeStep]}/>;
    }
    ...
    return (
        <>
            <ToastContainer/>
            <ProgressBar progress={(100 / productBasicFields.length) * (activeStep + 1)}/>
